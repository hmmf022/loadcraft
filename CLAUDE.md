...
## Phase 1 実装状況と設計書との差分

Phase 1 プロトタイプは完了。設計書 (`docs/`) は全Phase分の完全仕様を記述しているため、現時点の実装との差分を以下に記録する。
...
- VoxelGrid の hasCollision ではなく AABB ボックス同士の比較で簡易実装
...
設計書 `docs/04-state-management.md` に定義されているが Phase 1 で未実装のもの:

| 項目 | 対応Phase | 備考 |
|------|----------|------|
| `moveCargo` | Phase 2 | ドラッグ移動に必要 |
| `rotateCargo` | Phase 3 | 任意回転に必要 |
| `dragState` / `setDragState` | Phase 2 | ドラッグ中のゴースト表示用 |
...
| モジュール | 設計書 | 対応Phase |
|-----------|--------|----------|
| `Raycaster` | `docs/03-rendering-engine.md` | Phase 2 (3D選択/クリック) |
...
| `ErrorBoundary` / `WebGPUFallback` | `docs/05-ui-components.md` | Phase 2 |
